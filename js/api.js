        ...
        if (endpoint.includes("/yoga-poses")) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    throw new Error(
                        errorText || `HTTP錯誤 ${response.status}: ${response.statusText}`
                    );
                }
            } catch (fetchError) {
                // 清除超時計時器
...
const yogaAPI = {
    ...
    async getYogaActionByTitle(title) {
        try {
            return await apiRequest(`/yoga-poses?search=${title}`, { method: "GET" }).then(json => {
                if (json.error) { throw json.error }
                let pose = json.items[0]
                if (!pose) throw 'pose not found'
                return pose
            })
        } catch (error) {
            console.error(`獲取瑜伽動作詳情(Title: ${title})失敗:`, error);
            // 使用本地備份數據
            // if (typeof yogaPoses !== "undefined") {
            //     console.log("使用本地數據作為備份");
            //     const pose = yogaPoses.find((p) => p.id === title);
            //     if (pose) {
            //         return pose;
            //     }
            // }
