document.addEventListener('DOMContentLoaded', initPoseDetail);

function initPoseDetail() {
    // 獲取URL參數中的瑜伽動作ID
    const urlParams = new URLSearchParams(window.location.search);
    const yogaId = parseInt(urlParams.get('id'));
    const yogaTitleText = (urlParams.get('title'));
    ...
    async function loadYogaPoseDetails() {
        try {
            ...
            try {
                ...
                const apiData = await API.yoga.getYogaActionByTitle(yogaTitleText);
            ...
            if (!yogaPose.image_url) yogaPose.image_url = 'https://via.placeholder.com/300x200?text=No+Image';
            ...
            if (yogaImage) {
                // 確保圖片URL是有效的
                const imageUrl = yogaPose.image || yogaPose.imageUrl || yogaPose.img || yogaPose.picture || yogaPose.image_url;
    ...
    function updateYogaPoseDetails() {
        ...
        yogaVideo.src = yogaPose.video_url.replace('/watch?v=', '/embed/')
