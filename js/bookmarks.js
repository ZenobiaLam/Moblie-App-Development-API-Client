...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadBookmarks() {
        try {
            ...
            console.log('認證檢查結果:', authResult);
            const { isLoggedIn, user } = authResult;
            ...
            console.log('API返回收藏列表:', bookmarks);
            ...
            for (const bookmark of bookmarks) {
                try {
                    // 獲取瑜伽動作詳情
                    console.log('獲取收藏項目詳情, ID:', bookmark.item_id);
                    const yogaAction = await API.yoga.getYogaActionById(bookmark.item_id);
                    console.log('獲取到瑜伽動作詳情:', yogaAction);
